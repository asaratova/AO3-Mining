...
def compile_comments(bs):
    # only include comments on the first level (no replies)
    comments = bs.find_all('li', {'class': 'comment group odd'})
    l_comments = []
    if len(comments) > 0:
        for i in comments:
...
def turn_page_comments(bs, work_id, headers, start_page=1):
    page_limit = int(
        bs.find('li', {'class': 'next'}).previous_sibling.previous_sibling.text)
    print('** We have a total of', page_limit, 'pages of comments')
    out = ''
    for i in range(start_page, page_limit+1):
        url = 'https://archiveofourown.org' + work_id + '?page=' + \
            str(i) + '&show_comments=true&view_adult=true&view_full_work=true'
        try:
            req = urllib.request.Request(url, headers=headers)
            resp = urllib.request.urlopen(req)
            bs = BeautifulSoup(resp, 'lxml')
